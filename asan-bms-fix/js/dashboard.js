 ...
 * 업데이트: 2026-01-14 | 사업기간: 2023.08 ~ 2026.12 (연장승인)
...
const Utils = {
    ...
    getStatusColor: function(s) {
        var colors = {'완료':'#10B981','진행중':'#3B82F6','대기':'#6B7280','신규':'#8B5CF6','주의':'#EF4444'};
        return colors[s] || '#6B7280';
    },
    getStatusEmoji: function(s) {
        var emojis = {'완료':'✅','진행중':'🔄','대기':'⏸️','신규':'🆕','주의':'🔴'};
        return emojis[s] || '📋';
    },
    getMilestoneEmoji: function(type) {
        var emojis = {'보고':'📝','계약':'📄','착수':'🚀','점검':'🔍','마감':'📅'};
        return emojis[type] || '📌';
    },
    getMilestoneStatus: function(status) {
        var statuses = {'완료':'✅ 완료','예정':'📅 예정','진행중':'🔄 진행중'};
...
var BudgetDashboard = {
    ...
    renderFunding: function() {
        ...
        if (!el || !this.data.summary || !this.data.summary['재원별']) return;
        var f = this.data.summary['재원별'];
